...
impl Dfa {
    ...
    pub fn get_state_with_label(&self, label: u32) -> Option<&State> {
        self.states
            .iter()
            .find(|state| state.has_label(label))
    }
    ...
    fn append_transition_for_symbol(transitions: TransitionsLog, symbol: char) -> impl Fn(&State)-> Option<TransitionsLog> {
        move |state| state
    ...
    fn perform_transitions(&self, input: &String) -> TransitionsLog {
        input
            .chars()
            .into_iter()
            .fold(TransitionsLog::create(), |transitions, symbol| self
                .get_state_with_label(transitions.get_last_transition_or_(self.initial_state))
                .and_then(Dfa::append_transition_for_symbol(transitions.clone(), symbol))
...
mod tests {
    ...
    fn get_sample_dfa () -> Dfa {
        Dfa::new(0, vec![
            ...
            State::new(1, true, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 2),
            ])),
            State::new(2, true, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 3),
            ])),
    ...
    fn it_accepts_the_readme_dfa() {
        ...
        assert_eq!(result.to_string(), String::from("Input string: abaab
...
Input is ACCEPTED"))
    ...
    fn it_shows_rejected_states() {
        ...
        assert_eq!(result.to_string(), String::from("Input string: b
Current State  Input  Next State
0              b      3
Input is REJECTED"))
    }
    ...
    fn it_rejects_bad_input_strings() {
        ...
        assert_eq!(result.to_string(), String::from("Input string: acdfg
Current State  Input  Next State
0              a      1
Input is REJECTED"))
    }
    ...
    fn it_rejects_comparision_if_states_labels_are_not_the_same() {
        let dfa1= get_sample_dfa();
        let dfa2= Dfa::new(0, vec![
            State::new(5, false, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 3),
            ])),
            State::new(6, false, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 3),
            ])),
            State::new(7, false, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 3),
            ])),
            State::new(8, false, Transitions::new(vec![
                Transition::new('a', 1),
                Transition::new('b', 3),
            ])),
        ]);
