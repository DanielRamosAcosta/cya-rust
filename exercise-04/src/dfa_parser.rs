...
pub fn get_initial_state (string: &String) -> Option<u32> {
    string.chars().skip(2).take(1).collect::<String>().parse::<u32>().ok()
}

pub fn get_body (string: &String) -> String {
    string.chars().skip(4).take(string.len() - 4).collect::<String>()
}
...
pub fn parse_state (string: &String) -> State {
    let elements = string.split(' ').map(|str| String::from(str)).collect::<Vec<String>>();
    ...
    State::new(label.unwrap(), is_final.unwrap(), Transitions::new(transitions))
}
...
mod tests {
    ...
    fn it_accepts_the_readme_dfa() {
        ...
        let dfa2 = parse_dfa(String::from("4
0
0 0 2 a 1 b 3
1 1 2 a 1 b 2
2 1 2 a 1 b 3
3 0 2 a 3 b 3")).unwrap();
